-- ================================================================
-- ShoriBooks — Guest booking. Customers no longer need an account to book.
-- This REPLACES create_customer_appointment_safe with an auth-OPTIONAL
-- version (reusing all the existing validation/lock/deposit/insert logic):
--   • signed-in customer  → linked to their account (unchanged behaviour)
--   • guest (no session)  → a customer row with user_id = NULL, matched by
--                           phone; no trust account involved
-- Also grants EXECUTE to anon so an unauthenticated session can book, and
-- adds a light per-phone rate limit against guest spam.
-- ================================================================
...
BEGIN
  ...
  IF v_uid IS NOT NULL THEN
    -- Signed-in: prefer a row already linked to this user, else claim an
    -- unclaimed row matching this phone, else conflict if that phone is
    -- claimed by a DIFFERENT user, else create new (linked to the user).
    ...
    END IF;
  ELSE
    -- Guest: match an existing row by phone (reuse it so a returning guest
    -- keeps one history), else create an unclaimed row. Never overwrite the
    -- details of a row already claimed by a registered account.
    SELECT * INTO v_existing_by_phone
    FROM public.customers
    WHERE business_id = p_business_id AND phone = trim(p_customer_phone);
