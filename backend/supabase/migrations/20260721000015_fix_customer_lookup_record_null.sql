-- ================================================================
-- Shorivo -- FIX: "That record already exists" on repeat bookings.
--
-- create_customer_appointment_safe resolved the existing customer with
--   IF v_existing_by_phone IS NOT NULL   (and the same for _by_user)
-- but for a composite RECORD, `IS NOT NULL` is only TRUE when EVERY column
-- is non-null. A real customer row almost always has a null column (email,
-- whatsapp_number, user_id for guests, ...), so the "found" check evaluated
-- FALSE, the code fell through to INSERT, and the second booking hit the
-- customers_unique_phone (business_id, phone) constraint -> 23505.
--
-- Fix: test `.id IS NOT NULL` (id is non-null exactly when a row was found),
-- so an existing customer is reused/updated instead of re-inserted. Nothing
-- else changes. Additive + idempotent.
-- ================================================================
...
BEGIN
  ...
  -- Resolve the customer row. NOTE: use `.id IS NOT NULL` (not the RECORD
